'use client';

import { useState } from 'react';
import { AnimatePresence, LayoutGroup } from 'motion/react';
import { useUIStore } from '@/stores';
import { DEFAULT_RANK_LIMIT } from '@/constants/ranking';
import TradeRankCard from './TradeRankCard';
import type { TradeRankItem } from '@/types';

interface Props {
  items: TradeRankItem[];
}

const TRADE_SORT_OPTIONS = [
  { key: 'countTotal' as const, dir: 'desc' as const, label: '거래 많은 순' },
  { key: 'medianPrice' as const, dir: 'desc' as const, label: '매매가 높은 순' },
  { key: 'medianPrice' as const, dir: 'asc' as const, label: '매매가 낮은 순' },
];

export default function TradeRankList({ items }: Props) {
  const [showAll, setShowAll] = useState(false);
  const [sortIdx, setSortIdx] = useState(0);
  const selectApt = useUIStore((s) => s.selectApt);
  const sheetSnap = useUIStore((s) => s.sheetSnap);
  const setSheetSnap = useUIStore((s) => s.setSheetSnap);

  const option = TRADE_SORT_OPTIONS[sortIdx];
  const sorted = [...items].sort((a, b) => {
    const aVal = a[option.key];
    const bVal = b[option.key];
    return option.dir === 'desc' ? bVal - aVal : aVal - bVal;
  });

  const visibleItems = showAll ? sorted : sorted.slice(0, DEFAULT_RANK_LIMIT);
  const hasMore = sorted.length > DEFAULT_RANK_LIMIT;

  return (
    <div>
      <div className="flex items-center justify-between mb-3">
        <h3 className="text-sm font-bold text-slate-800 md:text-base">
          매매 단지 랭킹
          <span className="ml-1.5 text-xs font-medium text-slate-400">{items.length}개 단지</span>
        </h3>
        <div className="relative">
          <select
            value={sortIdx}
            onChange={(e) => {
              setSortIdx(Number(e.target.value));
              setShowAll(false);
            }}
            className="appearance-none bg-slate-50 border border-slate-200 rounded-lg pl-3 pr-7 py-1.5 text-xs font-medium text-slate-600 focus:outline-none focus:ring-1 focus:ring-blue-300 cursor-pointer"
          >
            {TRADE_SORT_OPTIONS.map((opt, idx) => (
              <option key={idx} value={idx}>
                {opt.label}
              </option>
            ))}
          </select>
          <svg
            width="12"
            height="12"
            viewBox="0 0 24 24"
            fill="none"
            stroke="currentColor"
            strokeWidth="2.5"
            strokeLinecap="round"
            strokeLinejoin="round"
            className="absolute right-2 top-1/2 -translate-y-1/2 text-slate-400 pointer-events-none"
          >
            <path d="m6 9 6 6 6-6" />
          </svg>
        </div>
      </div>

      {sorted.length === 0 ? (
        <div className="rounded-2xl border border-slate-100 bg-white p-6 text-center">
          <p className="text-sm text-slate-400 font-medium">해당 면적대에 거래 데이터가 없습니다</p>
        </div>
      ) : (
        <LayoutGroup>
          <div className="space-y-2.5">
            <AnimatePresence>
              {visibleItems.map((item, idx) => (
                <TradeRankCard
                  key={item.aptName}
                  item={item}
                  rank={idx + 1}
                  onClick={() => {
                    selectApt(item.aptName);
                    if (sheetSnap === 'full') setSheetSnap('half');
                  }}
                />
              ))}
            </AnimatePresence>
          </div>
        </LayoutGroup>
      )}

      {hasMore && !showAll && (
        <button
          onClick={() => setShowAll(true)}
          className="w-full mt-3 py-2.5 rounded-xl border border-slate-200 bg-white text-sm font-medium text-slate-500 hover:text-slate-700 hover:border-slate-300 transition-colors"
        >
          나머지 {sorted.length - DEFAULT_RANK_LIMIT}개 단지 더보기
        </button>
      )}
    </div>
  );
}
