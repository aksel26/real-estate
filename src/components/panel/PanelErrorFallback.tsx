'use client';

import { motion } from 'motion/react';
import Button from '@/components/ui/Button';

interface PanelErrorFallbackProps {
  error: Error;
  resetErrorBoundary: () => void;
  hasPreviousData?: boolean;
}

export default function PanelErrorFallback({
  error: _error,
  resetErrorBoundary,
  hasPreviousData = false,
}: PanelErrorFallbackProps) {
  return (
    <motion.div
      initial={{ opacity: 0, y: 6 }}
      animate={{ opacity: 1, y: 0 }}
      transition={{ duration: 0.25, ease: 'easeOut' }}
      className="flex flex-col items-center justify-center gap-3 px-4 py-8 text-center"
      role="alert"
    >
      <div className="flex items-center justify-center w-10 h-10 rounded-xl bg-red-50">
        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" aria-hidden="true">
          <path
            d="M12 9v4M12 17v.5"
            stroke="#ef4444"
            strokeWidth="1.75"
            strokeLinecap="round"
          />
          <path
            d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"
            stroke="#ef4444"
            strokeWidth="1.75"
            strokeLinejoin="round"
          />
        </svg>
      </div>

      <div className="space-y-1">
        <p className="text-sm font-semibold text-slate-800">데이터를 불러올 수 없습니다</p>
        {hasPreviousData && (
          <p className="text-xs text-amber-600 font-medium">이전 데이터를 표시 중입니다</p>
        )}
      </div>

      <Button variant="secondary" size="sm" onClick={resetErrorBoundary}>
        다시 시도
      </Button>
    </motion.div>
  );
}
