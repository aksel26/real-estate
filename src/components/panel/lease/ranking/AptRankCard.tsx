...
import type { AptRankItem } from '@/types';
...
export default function AptRankCard({ item, rank, onClick }: Props) {
  return (
    <motion.button
      layout
      onClick={onClick}
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="mt-1.5 flex items-center gap-2 text-[12px] font-semibold text-slate-500">
            ...
            <span className="bg-blue-50 px-2 py-0.5 rounded text-blue-600">
              전세 {item.countJeonse}건
            </span>
            <span className="bg-violet-50 px-2 py-0.5 rounded text-violet-600">
              월세 {item.countMonthly}건
            </span>
          </div>

          {/* 대표 가격 */}
          <div className="mt-2 flex items-center gap-3 text-[13px] font-medium text-slate-700">
            {item.countJeonse > 0 && (
              <span>
                전세 <span className="font-bold text-slate-900">{formatPrice(item.medianDeposit)}</span>
              </span>
            )}
            {item.countMonthly > 0 && (
              <span>
                ...
                <span className="font-bold text-slate-900">
                  {formatPrice(item.medianMonthlyDeposit)}/{formatPrice(item.medianMonthlyRent)}
                </span>
              </span>
            )}
          </div>
